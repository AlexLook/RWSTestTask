...
/// </summary>
/// <remarks>
/// Источник: https://github.com/agabani/DijkstraAlgorithm/tree/master
/// </remarks>
public class PathFinder {
    ...
    public Path FindShortestPath(Node origin, Node destination) {
        ...
        return Build(origin, destination, Process(origin, destination, graph));
    }
    ...
    private static List<Record> Process(Node origin, Node destination, Graph graph) {
        ...
        do {
            if (IsPathingNotPossible(currentRecord))
                throw new PathNotFoundException("No paths have been found.");
    ...
    private static Record NextRecordToProcess(IEnumerable<Record> records,
                                              ICollection<Node>   visited,
                                              Node                destination) {
    ...
    private static IEnumerable<PathSegment> PathSegments(Node         destination,
                                                         List<Record> records) {
    ...
    private static PathSegment CreateSegment(Record currentRecord) {
        return PathSegment.Create(currentRecord.PreviousVertex, currentRecord.Vertex,
            currentRecord.PreviousVertex
                         .Links
                         .Single(link => link.Destination == currentRecord.Vertex).Weight);
    }
